...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($end_date < $start_date) {
        ...
        try {
            // Get part details
            $stmt = $pdo->prepare("SELECT price FROM inventory WHERE part_id = ?");
            $stmt->execute([$part_id]);
            $part = $stmt->fetch(PDO::FETCH_ASSOC);
            
            // Calculate promo price
            if($discount_type == 'percentage') {
                $promo_price = $part['price'] - ($part['price'] * $discount_value / 100);
            } else {
                $promo_price = $part['price'] - $discount_value;
            }
...
<body>
  ...
  <div class="container">
    <a href="promo.php" class="back-link">Back to Promo Management</a>
    
    <div class="form-card">
      <div class="form-header">
        <h1>🏷️ Add New Promo</h1>
      ...
      <form method="POST" action="" id="promoForm">
        ...
        <div class="btn-group">
          ...
          <button type="submit" class="btn btn-primary">🏷️ Create Promo</button>
        </div>
      </form>
    </div>
  </div>
